    ...
    try:
        ...
        for word in words:
            if any(indicator in word for indicator in org_indicators):
                ...
                matches = sheets_db.find_org(word)
                if matches:
...
def get_pipeline_insights(sheets_db=None) -> dict:
...
def get_combined_context(query: str, sheets_db=None, email_generator=None) -> dict:
